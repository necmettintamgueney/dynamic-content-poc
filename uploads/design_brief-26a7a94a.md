...
- `design_brief_branding.md` -- Delivery Hero branding: colors, typography (Outfit), logo, visual style
- `design_brief_dataset.json` -- 30 products with broken vendor titles and AI-enriched versions at 4 tiers, category predictions, correction patterns
- `dh-logo.svg` -- Delivery Hero icon logo (red)

## Branding

Use the Delivery Hero branding package in `design_brief_branding.md`. Key points:
- **Font:** Outfit (Regular 400, SemiBold 600, Bold 700) -- available on Google Fonts
- **Primary color:** DH Red `#D61F26`
- **Logo:** `dh-logo.svg` (red arrow icon)
- Light or dark theme is fine; both palettes are provided in the branding file

## Interactive / Dynamic Requirements

This should NOT feel like a static presentation or slide deck. The demo sections should be interactive and dynamic:

- **Before/after title transformations** should animate or transition visually. The viewer should see the broken vendor title morph into the enriched version, not just see two text blocks side by side.
- **Tier comparison** should be interactive: let the viewer toggle between tiers (Lite / Pro / Plus / Enterprise) and see how the same product gets different enrichment. The dataset has 4-tier enrichment for every product.
- **Product examples** should be selectable or browsable. The viewer picks a product and sees its journey through the pipeline, not a predetermined static example.
- **The closed-loop diagram** should animate to show the continuous cycle. Nodes should be clickable or hoverable to reveal details.
- **Category prediction examples** should show correct vs. wrong predictions dynamically, with the ability to see why a prediction failed and how the feedback loop would fix it.
- **Progress/resolution tracking** should be visual. As solutions are introduced, the problems from earlier should visually resolve (checkmarks, status changes, color shifts) so the viewer feels the loop closing.

The goal is that a presenter can walk someone through this interactively, clicking and exploring, not just reading slides.
